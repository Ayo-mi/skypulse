...
- `carrier_capacity_ranking` — carrier leaderboard for a market, ranked by `total_capacity_added_seats` DESC (sum of POSITIVE capacity contributions only — added seats from growth, gauge_up, first_observed_in_dataset, re_observed_after_gap rows). Signed net change is also returned as `total_capacity_change_abs`. Each ranked carrier includes a `top_routes` array (up to 3 routes that drove their ranking), so a single call answers both "who added the most?" and "which routes drove the gains?" — even in contraction quarters where every major carrier nets negative. `aircraft_category` defaults to `"all"` (cross-fleet ranking) and accepts `narrowbody / widebody / regional_jet / turboprop / other` for fleet-specific queries. `period` is optional; omit for the all-recent-periods aggregate.
...
- For `carrier_capacity_ranking`, ALWAYS leave `aircraft_category` at its default of `"all"` unless the user explicitly asks for a specific fleet. Passing `"all"` returns the cross-fleet ranking in ONE call; enumerating fleet values to reconstruct the same view costs 4-5 calls and ~80s of LLM synthesis.
- `new_route_launches` and `frequency_losers` cap the default response at 30 rows so hub-airport answers stay small enough for fast LLM synthesis. Read `total_available` to know the full match count; only pass `limit=100` when the user explicitly asks for the full list — reflexively raising it on hub airports adds 30+ seconds of LLM synthesis without improving the answer.
