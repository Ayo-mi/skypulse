...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      'SINGLE-CALL ANSWER for: "Which carriers added the most capacity at airport X (and which routes drove the gains)?". Historical carrier leaderboard for a market (airport IATA), ranked by `total_capacity_added_seats` DESC — the SUM of POSITIVE capacity contributions only (added seats from growth, gauge_up, first_observed_in_dataset, and re_observed_after_gap rows). This means the flagship "Which carriers added the most capacity at JFK in 2026-Q1?" prompt always returns a useful gainers ranking even in contraction quarters where every major carrier nets negative. The signed net change is also returned as `total_capacity_change_abs` for agents that want it. EACH carrier in the response includes a `top_routes` array of up to 3 routes that contributed most to that carrier\'s capacity change — agents should NOT additionally call new_route_launches to learn which routes drove the gains, that information is already inline. `routes_gained` counts first_observed_in_dataset / re_observed_after_gap / growth / gauge_up; `routes_lost` counts suspension / reduction / gauge_down. Both `aircraft_category` (narrowbody / widebody / regional_jet / turboprop) and `period` (e.g. "2026-Q1") are OPTIONAL — when omitted you get the all-aircraft, all-recent-periods aggregate in ONE call (do not enumerate categories to reconstruct an all-aircraft view). Source: BTS T-100 Segment, 3-6 month publication lag. Good prompts: "Which carriers added the most capacity at JFK in 2026-Q1?" (single call — top_routes shows you exactly which routes drove the gains), "Rank carriers at DFW by total seat capacity change in 2026-Q1".',
