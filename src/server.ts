...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      'SINGLE-CALL ANSWER for: "What new routes appeared at airport X in BTS data?". Routes first observed (or re-observed after a gap) at a given airport in the BTS T-100 dataset. DEFAULT INVOCATION: pass only `airport` (and optionally `period`) — DO NOT pass `limit`. The default limit of 30 is the correct value for nearly every prompt; it returns the 30 most consequential new routes (ranked by current_inferred_seats DESC) and keeps hub-airport responses fast for the LLM to synthesize. Only set limit higher (max 100) when the user EXPLICITLY asks for "the full list" or "all new routes" — passing limit=100 reflexively on hub airports adds 30+ seconds of LLM synthesis time without improving the answer. The response includes `total_available` so you can see if the list was trimmed; that count alone is usually enough to answer "how many" questions without re-fetching the full list. IMPORTANT: change_type values are dataset observations, NOT confirmed real-world launches — `first_observed_in_dataset` means "earliest BTS quarter this carrier-route appears in our window" (which may post-date the marketing launch by months), and `re_observed_after_gap` means "activity resumed after ≥1 quarter of zero T-100 reports" (could be a true resumption, a seasonal route, or a reporting gap). effective_date is the BTS quarter midpoint, not the calendar launch day. Supports `period` filter (e.g. "2025-Q3") for quarterly scoping. Do NOT also call carrier_capacity_ranking afterwards — this tool already lists the first-observed routes at the airport. Good prompts: "What routes were first observed in BTS data at ORD in 2025-Q3?" → call once with {airport:"ORD", period:"2025-Q3"} (no limit); "Show me all 91 new routes at LAS in 2026-Q1" → call once with {airport:"LAS", period:"2026-Q1", limit:100} (only when the user explicitly asks for the full list).',
  ...
  },
  {
    ...
    description:
      'SINGLE-CALL ANSWER for: "Which carriers added the most capacity at airport X (and which routes drove the gains)?". DEFAULT INVOCATION: pass only `market` (and optionally `period`) — leave `aircraft_category` at its default of "all". The "all" value returns the cross-fleet ranking in ONE call and is the correct answer for the flagship gainers prompt; only set aircraft_category to a specific fleet (narrowbody / widebody / regional_jet / turboprop / other) when the user explicitly asks for that fleet (e.g. "narrowbody gainers at MIA"). NEVER enumerate aircraft_category values to reconstruct an all-fleet view — pass "all" once instead. Historical carrier leaderboard for a market (airport IATA), ranked by `total_capacity_added_seats` DESC — the SUM of POSITIVE capacity contributions only (added seats from growth, gauge_up, first_observed_in_dataset, and re_observed_after_gap rows). This means even in contraction quarters where every major carrier nets negative, the gainers ranking is still meaningful. The signed net change is also returned as `total_capacity_change_abs`. EACH carrier in the response includes a `top_routes` array of up to 3 routes that contributed most to that carrier\'s capacity change — agents should NOT additionally call new_route_launches, that information is already inline. `routes_gained` counts first_observed_in_dataset / re_observed_after_gap / growth / gauge_up; `routes_lost` counts suspension / reduction / gauge_down. Source: BTS T-100 Segment, 3-6 month publication lag. Good prompts: "Which carriers added the most capacity at JFK in 2026-Q1?" → call once with {market:"JFK", period:"2026-Q1"} (aircraft_category defaults to "all"); "Which carriers added the most narrowbody capacity at MIA?" → call once with {market:"MIA", aircraft_category:"narrowbody"}.',
